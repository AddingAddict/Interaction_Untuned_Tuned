...
def predict_networks(prms,rX,cA,CVh):
    ...
    if cA == 0 or prms.get("basefrac",0)==1:
        ...
        normC[:,0] = gauss(oris[None,:],res_dict["Crm"][:2,-1,None],res_dict["Crp"][:2,-1,None],
                           res_dict["sCr"][:2,-1,None]) /\
                     gauss(oris[None,:],res_dict["Crm"][:2, 0,None],res_dict["Crp"][:2, 0,None],
                           res_dict["sCr"][:2, 0,None])
        normC[:,1] = gauss(oris[None,:],res_dict["Crm"][2:,-1,None],res_dict["Crp"][2:,-1,None],
                           res_dict["sCr"][2:,-1,None]) /\
                     gauss(oris[None,:],res_dict["Crm"][2:, 0,None],res_dict["Crp"][2:, 0,None],
                           res_dict["sCr"][2:, 0,None])
        normC[:,2] = gauss(oris[None,:],res_dict["Cdrm"][:,-1,None],res_dict["Cdrp"][:,-1,None],
                           res_dict["sCdr"][:,-1,None]) /\
                     gauss(oris[None,:],res_dict["Cdrm"][:, 0,None],res_dict["Cdrp"][:, 0,None],
    ...
    Sigdp = Sigdb + dmft.struct_fact(0,sWCdr,sCdr)*SigW*(Cdrp-Cdrm)
    Sigdb = Sigdb + dmft.struct_fact(90,sWCdr,sCdr)*SigW*(Cdrp-Cdrm)
    sSigd2 = sWCdr**2
    ...
    for i in range(2):
        ...
        Σrs[i,2] = np.fmax(gauss(oris,Cdrm[i],Cdrp[i],sCdr2[i]) - μrs[i,2]**2,0)
